...
class table_tom_tongcheng_district extends discuz_table{
	public function __construct() {
        parent::__construct();
		$this->_table = 'tom_tongcheng_district';
		$this->_pk    = 'id';
	}
    ...
    public function fetch_by_level1_name($name,$field='*') {
		return DB::fetch_first("SELECT $field FROM %t WHERE name LIKE %s AND level=1 ", array($this->_table,'%'.$name.'%'));
	}
