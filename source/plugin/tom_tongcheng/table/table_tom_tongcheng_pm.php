...
class table_tom_tongcheng_pm extends discuz_table{
	public function __construct() {
        parent::__construct();
		$this->_table = 'tom_tongcheng_pm';
		$this->_pk    = 'id';
	}
    ...
    public function fetch_all_newnum($condition) {
        $return = DB::fetch_first("SELECT SUM(new_num) AS newnum FROM ".DB::table($this->_table)." WHERE 1 $condition ");
		return $return['newnum'];
	}
