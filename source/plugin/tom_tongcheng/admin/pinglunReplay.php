...
$get_list_url_value = get_list_url("tom_tongcheng_admin_pinglun_reply_list");
...
if($_GET['formhash'] == FORMHASH && $_GET['act'] == 'del'){
    ...
    C::t('#tom_tongcheng#tom_tongcheng_pinglun_reply')->delete($_GET['id']);
    
    cpmsg($Lang['act_success'], $modListUrl, 'succeed');
}else{
    
    set_list_url("tom_tongcheng_admin_pinglun_reply_list");
    
    $pagesize = 100;
    $page = intval($_GET['page'])>0? intval($_GET['page']):1;
    $start = ($page-1)*$pagesize;	
    $count = C::t('#tom_tongcheng#tom_tongcheng_pinglun_reply')->fetch_all_count("");
    $replyList = C::t('#tom_tongcheng#tom_tongcheng_pinglun_reply')->fetch_all_list("","ORDER BY reply_time DESC",$start,$pagesize);
    showtableheader();
