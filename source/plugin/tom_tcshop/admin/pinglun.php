...
$get_list_url_value = get_list_url("tom_tcshop_admin_pinglun_list");
...
if($_GET['formhash'] == FORMHASH && $_GET['act'] == 'del'){
    ...
    C::t('#tom_tcshop#tom_tcshop_pinglun')->delete($_GET['id']);
    C::t('#tom_tcshop#tom_tcshop_pinglun_reply')->delete_pinglun_id($_GET['id']);
    
    cpmsg($Lang['act_success'], $modListUrl, 'succeed');
}else{
    
    set_list_url("tom_tcshop_admin_pinglun_list");
    
    $pagesize = 100;
    $page = intval($_GET['page'])>0? intval($_GET['page']):1;
    $start = ($page-1)*$pagesize;	
    $count = C::t('#tom_tcshop#tom_tcshop_pinglun')->fetch_all_count("");
    $pinglunList = C::t('#tom_tcshop#tom_tcshop_pinglun')->fetch_all_list("","ORDER BY ping_time DESC",$start,$pagesize);
    showtableheader();
